...
class RawFileBrowser extends React.Component {
  ...
  handleActionBarAddFolderClick = (event) => {
    ...
    this.setState(prevState => {
      let addKey = ''
      console.log("1" + prevState.selection + "2")
      if (prevState.selection && prevState.selection !== '/' && prevState.selection !== ' ' && prevState.selection !== '') {
    ...
    if (this.state.nameFilter) {
      ...
      files.map((file) => {
        ...
        terms.map((term) => {
          ...
          else if (file.key.toLowerCase().trim().indexOf(term) === -1 && 
                   citation.quotes.findIndex(quote => quote != null && 
                    ((quote.quote && quote.quote.toLowerCase().trim().indexOf(term) > -1)
                    || (quote.comment && quote.comment.toLowerCase().trim().indexOf(term) > -1))) === -1) {
             skip = true
          }
        })
        if (skip) {
        ...
        if (!contents.length) {
          if (this.state.nameFilter) {
            ...
            contents = (
              <tr>
                <td colSpan={100}>
                <this.props.noFilesMessage /> {/* Could be {this.props.noFIlesMessage} */}
                </td>
              </tr>
            )
          }
